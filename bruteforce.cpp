...
{	  
	...
	vector<int> newSet2(data.num_base_stations);	
        ...
        vector<int>::iterator  it2 = set_difference(data.antennas.at(setNum).begin(), data.antennas.at(setNum).end(), 
			 	      dc.begin(), dc.end(),
			 				    newSet2.begin());
	...
	it2 = set_union(dc.begin(), dc.end(), newSet4.begin(), newSet4.end(), newSet2.begin());
	newSet2.erase(it2, newSet2.end());	 				   
       ...
       //TODO: only for debugging purposes, remove eventually
       cout << "set num " << setNum << " level " << level << " set_antenna size " << set_antenna.size() << 
       		" set_base size " << set_base.size() << " onecoveredNum " << onecoveredNum << endl;
